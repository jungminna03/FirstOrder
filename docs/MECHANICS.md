...
#### 레버 1: 브릭 하강 속도 (가장 먼저 체감)
- 초기: **50 px/s**
- 30s마다 **+10 px/s** 선형 증가
- **180s(틱 6) 이후 110 px/s로 고정**
...
#### 레버 2: 브릭 HP (중반부터 체감)
- 초기: **1 HP** (90s까지 유지)
- **90s(틱 3)부터** 증분이 +2씩 커지는 가속 증가
- 180s 이후에도 **계속 증가** (속도·갯수가 캡된 뒤 유일한 난이도 상승 수단)
...
#### 레버 3: 브릭 밀도 (마지막에 체감, 180s에 포화)
- `density = clamp(0.08 + (waveCount - 1) × 0.012, 0.08, 1.0)`
- 웨이브 1: ~1개 | 웨이브 40: ~5개 | **웨이브 ~77(≈180s): 꽉 찬 행 (10개)**
- 웨이브당 최소 1개 보장
- **180s 이후 밀도 1.0 고정**
